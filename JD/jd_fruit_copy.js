...
let name = $.getdata('jd_fruit_name_z');
//Node.js用户请在jdCookie.js处填写京东ck;
const jdCookieNode = $.isNode() ? require('./jdCookie.js') : '';

//ios等软件用户直接用NobyDa的jd cookie
const cookie = jdCookieNode.CookieJD ? jdCookieNode.CookieJD : $.getdata('CookieJD_fruit');
...
const boxShareCodeArr = ['jd_fruit1_z', 'jd_fruit2_z', 'jd_fruit3_z', 'jd_fruit4_z'];
...
function* step() {
    ...
    if (farmInfo.farmUserPro) {
        ...
        if (farmInfo.code === '3') {
            ...
            $.setdata('', 'CookieJD_fruit');//cookie失效，故清空cookie。
            $.done();
            return
        } else {
